...
All users are in ou=users - group membership determines their role.
...
- ldapUserDN: The user's full LDAP DN (e.g., cn=user,ou=users,dc=hermes,dc=local)
- isRelay: Boolean - member of cn=relays,ou=groups
- isMailbox: Boolean - member of cn=mailboxes,ou=groups
- isAdmin: Boolean - member of cn=admins,ou=groups
--->
...
<!---
SEARCH FOR USER BY EMAIL IN ou=users
All users are in ou=users - role is determined by group membership.
--->

<cfexecute name="/usr/local/bin/docker"
    arguments="exec hermes_ldap ldapsearch -Y EXTERNAL -H ldapi://%2Fvar%2Frun%2Fslapd%2Fldapi -b 'ou=users,dc=hermes,dc=local' '(mail=#userEmail#)' dn cn"
...
<cfif ldapUserFound>
    ...
    <cfif dnLine NEQ "">
        <!--- Extract the full DN (everything after "dn: ") --->
        <cfset ldapUserDN = Trim(Replace(dnLine, "dn: ", ""))>
...
<cfif ldapUserFound AND ldapUsername NEQ "" AND ldapUserDN NEQ "">

    <!--- CHECK RELAYS GROUP --->
    <cfexecute name="/usr/local/bin/docker"
        arguments="exec hermes_ldap ldapsearch -Y EXTERNAL -H ldapi://%2Fvar%2Frun%2Fslapd%2Fldapi -b 'cn=relays,ou=groups,dc=hermes,dc=local' '(member=#ldapUserDN#)' dn"
    ...
    <cfexecute name="/usr/local/bin/docker"
        arguments="exec hermes_ldap ldapsearch -Y EXTERNAL -H ldapi://%2Fvar%2Frun%2Fslapd%2Fldapi -b 'cn=mailboxes,ou=groups,dc=hermes,dc=local' '(member=#ldapUserDN#)' dn"
    ...
    <!--- CHECK ADMINS GROUP --->
    <cfexecute name="/usr/local/bin/docker"
        arguments="exec hermes_ldap ldapsearch -Y EXTERNAL -H ldapi://%2Fvar%2Frun%2Fslapd%2Fldapi -b 'cn=admins,ou=groups,dc=hermes,dc=local' '(member=#ldapUserDN#)' dn"
