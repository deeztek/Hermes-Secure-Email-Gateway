...
select id, acme_certificate, mailbox_domain, subdomain, ip from mailbox_domains_sans
  </cfquery>
...
select distinct(acme_certificate) from mailbox_domains_sans where ip = 'YES'
</cfquery>
...
<cfquery name="create_validated_hash" datasource="hermes">
  select subdomain from mailbox_domains_sans where acme_certificate = '#acme_certificate#' and ip = 'YES' order by subdomain asc
</cfquery>
...
<!--- Count the number of subdomains from previous create_validated_hash query from above and add the certificate domain --->
<cfset totalsubdomain = #create_validated_hash.recordcount# + 1>

<cfoutput>Total number of SANs is: #totalsubdomain#</cfoutput><br>

<cfif #totalsubdomain# LT 100>

<cfoutput>Thhe number of SANs is below 100. Proceeding with certificate request...</cfoutput><br>
...
update mailbox_domains_sans set dns = 'YES', dns_result_msg = 'SUCCESS: Successfully Received SAN Certificate', dns_result_datetime = '#datenow# #timenow#' where acme_certificate = '#acme_certificate#'
</cfquery>
