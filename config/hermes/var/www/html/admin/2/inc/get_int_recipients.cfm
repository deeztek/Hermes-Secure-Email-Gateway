...
<cfquery name="getrecipients" datasource="hermes">
select recipients.id, recipients.id as theID, recipients.id as theOtherID, recipients.recipient, recipients.policy_id, recipients.enforce_mfa, recipients.auth_type, recipients.remoteauth_domain, policy.policy_name, user_settings.report_enabled as report_enabled, user_settings.train_bayes as train_bayes_val, user_settings.download_msg as download_msg_val, if(user_settings.train_bayes = 1, 'YES', 'NO') as train_bayes, if(user_settings.download_msg = 1, 'YES', 'NO') as download_msg, if(recipients.pdf_enabled = 1, 'YES', 'NO') as pdf_enabled, if(recipients.smime_enabled = '1', 'YES', 'NO') as smime_enabled, if(recipients.pgp_enabled = 1, 'YES', 'NO') as pgp_enabled, if(recipients.digital_sign = '1', 'YES', 'NO') as digital_sign, if(recipient_certificates.user_id is NULL, 'NO', 'YES') as cert, if(recipient_keystores.user_id is NULL, 'NO', 'YES') as keystore
