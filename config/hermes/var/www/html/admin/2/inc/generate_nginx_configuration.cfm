...
<!--- Fall back to the always-present bootstrap cert when the ACME cert files
     aren't on THIS box (cross-host restore without the LE store, failed/pending
     issuance, expired+pruned cert). A regen must NEVER emit a cert path that
     doesn't exist -- nginx refuses to start (BIO_new_file) and takes the whole
     console down. Re-issuing the cert restores the real path on the next regen. --->
<cfif NOT (FileExists(certpath) AND FileExists(keypath))>
<cfset certpath = "/opt/hermes/ssl/bootstrap_hermes.bundle.pem">
<cfset keypath = "/opt/hermes/ssl/bootstrap_hermes.key">
</cfif>
