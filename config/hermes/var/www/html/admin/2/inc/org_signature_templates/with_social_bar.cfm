<!---
ORGANIZATIONAL SIGNATURE TEMPLATE: With Social Bar (#226 Phase 2A)

Layout:
  Name (bold, larger)
  Title
  ---- accent thin line ----
  Phone | Email | Website   (single line, separators)
  Address (optional, smaller)
  =================
  [in] [tw] [gh] [ig] [fb] [yt]    <- horizontal social bar (full width)

Vertical stack with prominent social media icon strip at the bottom -
the "social-first" template for marketing / brand-heavy teams.
--->

<cfscript>
template = {
    key:         "with_social_bar",
    name:        "With Social Bar",
    ...
    fields: [
        ...
        { name: "show_socials",   label: "Show social bar", type: "checkbox", default: true,
          help: "The horizontal social bar at the bottom is the focus of this template. Leave any individual URL below blank to hide that platform's icon - only the platforms with a URL appear in the bar." },
...
<div style="color:##555;">#fields.user_title#</div>
</cfif>
<div style="border-top:2px solid #fields.accent_color#;margin:8px 0;width:80px;"></div>
<cfset contactBits = []>
...
<div style="margin:10px 0;"><a href="#fields.cta_url#" style="display:inline-block;background:#fields.accent_color#;color:##ffffff;text-decoration:none;font-weight:bold;padding:7px 14px;border-radius:4px;font-size:12px;">#fields.cta_label#</a></div>
</cfif>
    ...
    <cfif Len(Trim(sp.url))>
        ...
        <cfif Len(iconUri)>
            <cfif socialFirst>
                ...
                <div style="margin-top:10px;padding:8px 10px;background:#fields.accent_color#;border-radius:4px;">
            </cfif>
            <a href="#sp.url#" style="text-decoration:none;margin-right:8px;display:inline-block;"><img src="#iconUri#" alt="" width="22" height="22" style="vertical-align:middle;border:0;filter:brightness(0) invert(1);" /></a>
        </cfif>
    </cfif>
</cfloop>
