...
SIEVE RULE ACTIONS HANDLER
Handles add, edit, delete, toggle, and reorder actions for global sieve rules.
--->
...
<cfif action EQ "add_rule">
    ...
    <cfquery name="getMaxOrder" datasource="hermes">
        SELECT COALESCE(MAX(rule_order), 0) + 1 AS next_order FROM sieve_rules WHERE scope = 'global'
    </cfquery>

    <cfquery datasource="hermes">
        INSERT INTO sieve_rules (scope, rule_name, rule_order, condition_field, condition_type, condition_value, action_type, action_value)
        VALUES (
          'global',
    ...
    <!--- Regenerate global sieve script --->
...
<cfelseif action EQ "edit_rule">
    ...
    <!--- Verify rule exists and is not system --->
    ...
    <cfquery datasource="hermes">
        ...
        <cfif getRule.is_system EQ 1>AND is_system = 0</cfif>
    </cfquery>
