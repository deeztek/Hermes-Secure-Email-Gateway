...
<cfif _certIdLookup.recordcount EQ 1
    ...
    <cfif _certRow.recordcount EQ 1>
        ...
        <cfelseif _certRow.type EQ "Acme">
            <cfset _acmeNginx = "/etc/letsencrypt/live/" & _certRow.file_name & "/fullchain.pem">
            <cfset _acmeKey   = "/etc/letsencrypt/live/" & _certRow.file_name & "/privkey.pem">
            <cfif FileExists(_acmeNginx) AND FileExists(_acmeKey)>
                <cfset hermesCertType = "Acme">
                <cfset hermesCertIsSnakeoil = false>
                <cfset hermesCertNginxPath = _acmeNginx>
                <cfset hermesCertKeyPath = _acmeKey>
            ...
            <!--- ACME cert configured in the DB but its files are NOT on this
                 box -- e.g. a cross-host restore that didn't carry the Let's
                 Encrypt store, a failed/pending issuance, or an expired+pruned
                 cert. Keep the bootstrap/system-cert paths set above so nginx
                 still loads instead of crashing on a missing file (BIO_new_file
                 takes the whole console down). Stays flagged Snakeoil so signing
                 callers skip. Re-issuing the cert restores the real paths on the
                 next regen. --->
            </cfif>
        </cfif>
    </cfif>
