...
<!--- READ THE LDAP REMOVEUSERGROUP MAILBOX TEMPLATE --->
...
<cftry>
    ...
    <cffile action="write"
        file="/opt/hermes/tmp/#customtrans3#_deleteuser_mailbox.ldif"
        output="#ldapDeleteLdif#"
        addNewLine="no">

    <cfexecute name="/usr/local/bin/docker"
        arguments="exec hermes_ldap ldapmodify -Y EXTERNAL -H ldapi://%2Fvar%2Frun%2Fslapd%2Fldapi -f /opt/hermes/tmp/#customtrans3#_deleteuser_mailbox.ldif"
    ...
    <!--- CLEANUP TEMP FILE --->
    <cfset fileToDelete = "/opt/hermes/tmp/#customtrans3#_deleteuser_mailbox.ldif">
    <cfif fileExists(fileToDelete)>
        <cffile action="delete" file="#fileToDelete#">
    </cfif>
    ...
    <cfelseif ldapDeleteError CONTAINS "ldap_delete:" OR ldapDeleteError CONTAINS "ldap_modify:">
        <!--- Real error --->
        <cfset m="LDAP Delete Mailbox User Failed: #ldapDeleteError#">
...
<cfcatch type="any">
    ...
    <cfif errorDetail CONTAINS "No such object" OR cfcatch.detail CONTAINS "No such object" OR cfcatch.message CONTAINS "No such object">
        <!--- User doesn't exist in LDAP, that's OK --->
    <cfelse>
        <cfset m="LDAP Delete Mailbox User: #cfcatch.message# | Detail: #cfcatch.detail# | LDAP Error: #errorDetail#">
        <cfinclude template="error.cfm">
        <cfabort>
    </cfif>
</cfcatch>
