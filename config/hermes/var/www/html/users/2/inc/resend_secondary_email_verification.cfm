...
<!--- GET CURRENT SECONDARY EMAIL --->
<cfquery name="getCurrentSettings" datasource="hermes">
    SELECT secondary_email, secondary_email_verified
    FROM user_settings
    WHERE email = <cfqueryparam cfsqltype="cf_sql_varchar" value="#session.email#">
</cfquery>

<cfif getCurrentSettings.recordcount EQ 0 OR getCurrentSettings.secondary_email EQ "">
    <cfset session.message = "No recovery email is set. Please add one first.">
    <cfset session.messageType = "warning">
    <cflocation url="user_settings.cfm" addtoken="no">
</cfif>

<cfif getCurrentSettings.secondary_email_verified EQ 1>
    <cfset session.message = "Your recovery email is already verified.">
    <cfset session.messageType = "info">
    <cflocation url="user_settings.cfm" addtoken="no">
</cfif>

<!--- GENERATE NEW VERIFICATION TOKEN (64 characters) --->
...
<cfquery datasource="hermes">
    ...
    SET secondary_email_token = <cfqueryparam cfsqltype="cf_sql_varchar" value="#verifyToken#">,
...
<cftry>
    <cfmail from="#getPostmaster.value#" to="#getCurrentSettings.secondary_email#" server="hermes_postfix_dkim" port="10026" subject="[Hermes SEG] Verify Your Recovery Email" type="html">
<div align="center">
...
<b>*** This link is valid for 24 hours ***</b>
    ...
    <cfset session.message = "A new verification email has been sent to <strong>#HTMLEditFormat(getCurrentSettings.secondary_email)#</strong>. Please check your inbox.">
