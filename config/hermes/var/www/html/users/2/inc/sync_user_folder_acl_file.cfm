...
<!---
SYNC USER FOLDER ACL FILE (Dovecot 2.4 vfile driver)

Rebuilds the per-folder /srv/mail/<domain>/<local>/<.subfolder>/dovecot-acl
file from the authoritative user_folder_shares table. Parallel to
admin/2/inc/sync_shared_mailbox_acl_file.cfm but targeting a SUBFOLDER's
Maildir path rather than the mailbox root.

Required inputs:
  - ownerUser : owner's email (full address, e.g. "dedwards@deeztek.org")
  - folderPath : folder path in IMAP form (e.g. "Work" or "Projects/Alpha").
                 Leading "INBOX/" is tolerated and stripped.

Maildir path conversion:
  - IMAP separator is "/" (inbox namespace). Maildir subdirs use "."
    prefixed and "." between levels.
  - "Work"             -> .Work
  - "Projects/Alpha"   -> .Projects.Alpha
  - "INBOX/Work"       -> .Work (leading INBOX/ stripped)
  - "INBOX" or ""      -> "" (root Maildir; ACL applies to INBOX itself)

vfile rights (RFC 4314 single-letter concatenated):
  can_read   -> lrs   (lookup, read, write-seen)
  can_write  -> wti   (write, write-deleted, insert — NB user_folder_shares
                       ties can_insert to can_write via the action handler)
  can_delete -> e     (expunge)

Non-fatal on failure: DB state is source of truth; next share/unshare on
the same folder retries the sync.
--->
...
<!--- Convert IMAP folder path to Maildir subdir. Trim any leading slash
     and strip INBOX/ prefix. If what's left is "INBOX" or empty, the
     target is the Maildir root itself (INBOX). --->
...
<!--- Pull all shares of THIS folder from the source of truth. --->
