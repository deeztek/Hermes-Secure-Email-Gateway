# Hermes SEG - URL Safe Links Implementation Plan
...
Rewrite URLs in inbound emails to route through a Hermes safe-link proxy endpoint. At click time, check the URL against threat intelligence services before redirecting to the destination. Strip rewritten URLs from outbound emails to prevent internal URLs from leaking externally.
...
Click-Time Flow:
  User clicks rewritten link in email client
    → Browser hits https://console/safe-link?url=<encoded>&sig=<HMAC>
...
Phase 2: At eom() callback:
  ...
  c. For inbound:
     ...
     - Rewrite each URL to safe-link format with HMAC signature
     - Reassemble MIME message
     - Call replacebody() with modified content
  d. For outbound:
     - Walk MIME parts
     - Find Hermes safe-link URLs
...
Rewritten: https://{console_host}/safe-link?url={base64url(original)}&exp={expiry}&sig={HMAC-SHA256}
```
...
**Location**: `/user-auth/safe_link.cfm` (public, no Authelia — same as quarantine release)
...
- [ ] Create `/user-auth/safe_link.cfm`
...
- [ ] Rate limiting on safe-link endpoint

---

## Security Considerations

1. **DKIM Invalidation**: URL rewriting modifies the message body, which invalidates any existing DKIM signature. This is by design (same as Microsoft Safe Links). The message will show DKIM=fail for the original sender's signature, but Hermes adds its own DKIM signature after rewriting.

2. **Recursive URL Unwrapping**: Attackers chain multiple vendor safe-link URLs together. The safe-link endpoint must recursively unwrap URLs up to a configurable depth limit (default 3) and flag excessive redirects.
...
4. **Open Redirect Prevention**: The safe-link endpoint must validate that the decoded URL is a proper HTTP/HTTPS URL, not a javascript: or data: URI.

5. **Rate Limiting**: The safe-link endpoint should be rate-limited to prevent abuse as a URL proxy.
...
| `config/hermes/var/www/html/user-auth/safe_link.cfm` | Click-time check endpoint |
