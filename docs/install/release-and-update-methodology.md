...
docker compose up -d         # always (idempotent: only restarts services whose config changed)
```
...
6. **Update `RELEASE-NOTES.md`**: add a section for `v<DATE>` listing every change.
