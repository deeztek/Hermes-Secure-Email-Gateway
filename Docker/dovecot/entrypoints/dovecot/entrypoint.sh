...
# Ensure global sieve directory exists for sieve_before scripts
mkdir -p /srv/mail/sieve/global
if [ ! -f /srv/mail/sieve/global/before.sieve ]; then
    # Create empty placeholder so Dovecot doesn't complain about missing script
    echo "## Hermes SEG Global Sieve Rules (placeholder - will be regenerated by Hermes)" > /srv/mail/sieve/global/before.sieve
fi
chown -R vmail:vmail /srv/mail/sieve
