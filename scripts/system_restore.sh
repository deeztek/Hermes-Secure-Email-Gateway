...
confirm_destructive() {
    ...
    bk_has_archive "ldap.ldif.gz"     && log "  - OpenLDAP directory (${LDAP_SUFFIX}) -- wiped + reloaded from backup"
    bk_has_archive "config.tar.gz"    && log "  - Config tier user-data subdirs only (DKIM/PGP/SSL keys, Let's Encrypt certs, custom templates, bayes corpus) -- overlaid into ${TIER_PATH[config]}"
    bk_has_archive "data.tar.gz"      && log "  - Data tier user-data subdirs only (sieve scripts, Authelia SQLite if any) -- overlaid into ${TIER_PATH[data]}"
    bk_has_archive "archive.tar.gz"   && log "  - Archive tier (${TIER_PATH[archive]}) -- overlaid from backup"
    bk_has_archive "vmail.tar.gz"     && log "  - Vmail tier (${TIER_PATH[vmail]}) -- overlaid from backup"
    bk_has_archive "nextcloud.tar.gz" && log "  - Nextcloud tier (${TIER_PATH[nextcloud]}) -- overlaid from backup"
    log ""
    log "Note: 'overlaid' = tar -xzf into the destination. Files at the"
    log "destination that aren't in the backup are preserved (tar can't"
    log "delete what it doesn't see). Existing files of the same name ARE"
    log "overwritten."
    log ""
