...
#ATTEMPT TO UMOUNT BUSY CIFS shares
/bin/umount.cifs -f -l /mnt/hermesbackups

#MOUNT BACKUPS shares
/bin/mount '//ovhfs.hosting.deeztek.com/shares/backups/relay' -t cifs -o uid=1000,gid=1000,vers=2.0,username='hermesbackup',domain='OVHFS',password='***REDACTED-CRED***' /mnt/hermesbackups/ 2>&1 | tee /opt/hermes/tmp/backup_mount.log
...
MYSQLUSER="root"
MYSQLPASS="EyY4t5cfBEKUYJFbSExP"
...
#PERFORM THE BACKUP
/usr/bin/rar a -m1 -mt8  $BACKUPS/$FILENAME-$TIMESTAMP.rar /etc/clamav/clamd.conf /etc/clamav-unofficial-sigs/user.conf /etc/postfix/main.cf /etc/postfix/networks /etc/postfix/networks.db /etc/postfix/postscreen_access.cidr /etc/postfix/postscreen_access.cidr.db /etc/postfix/relay_domains /etc/postfix/relay_domains.db /etc/postfix/relay_passwd /etc/postfix/relay_passwd.db /etc/postfix/relay_recipients /etc/postfix/relay_recipients.db /etc/postfix/sender_access /etc/postfix/sender_access.db /etc/postfix/tls_policy /etc/postfix/tls_policy.db /etc/postfix/transport /etc/postfix/transport.db /etc/postfix/virtual /etc/postfix/virtual.db /etc/postfix/bcc_maps /etc/postfix/bcc_maps.db /etc/amavis/mynetworks /etc/amavis/black.lst /etc/amavis/white.lst /etc/amavis/conf.d/15-av_scanners /etc/amavis/conf.d/50-user /etc/spamassassin/local.cf /opt/hermes/ssl /opt/hermes/backups /opt/hermes/CA /opt/hermes/dkim /opt/hermes/HermesExternalCACerts /opt/hermes/keys /opt/hermes/root_ca /opt/hermes/sa-bayes /opt/hermes/schedule /opt/hermes/scheduled_tasks /opt/hermes/.gnupg /mnt/data/amavis /var/www/schedule /var/www/html/schedule /var/www/WEB-INF/railo/scheduler/scheduler.xml /var/www/html/WEB-INF/lucee/scheduler/scheduler.xml $BACKUPS/dbbkup/hermes.sql $BACKUPS/dbbkup/syslog.sql $BACKUPS/dbbkup/djigzo.sql $BACKUPS/dbbkup/opendmarc.sql >> $BACKUPS/backuplog-$TIMESTAMP.log
...
/usr/bin/sendemail -f postmaster@deeztek.com -t postmaster@domain.tld -u "Success `hostname --fqdn` Backup" -m "Success `hostname --fqdn` Backup"  -s localhost
...
echo "[`date +%m/%d/%Y-%H:%M`] STEP 10 OF 9. COMPLETED SET BACKUP JOB STOPPED STATUS" >> $BACKUPS/backuplog-$TIMESTAMP.log
...
/usr/bin/sendemail -f postmaster@deeztek.com -t postmaster@domain.tld -u "Failed `hostname --fqdn` Backup" -m "Failed `hostname --fqdn` Backup. Unable to attach backup share. Attached backup mount log may assist in troubleshooting the problem"  -s localhost -a /opt/hermes/tmp/backup_mount.log
