...
#/usr/bin/rar a $BACKUPS/hermes_sysarch_enc_$TIMESTAMP.rar /etc/network/interfaces /etc/resolv.conf /etc/postfix /etc/amavis /etc/spamassassin /etc/hostname /etc/hosts /etc/mailname /etc/apache2/sites-enabled /etc/apache2/sites-available /opt/hermes /opt/scripts /var/www /usr/share/UUID /usr/share/UUID2 /usr/share/UUID3 /usr/share/djigzo/ADDITIONAL-NOTES.TXT /usr/share/djigzo/DOCS.TXT /mnt/data/amavis $BACKUPS/dbbkup/hermes.sql $BACKUPS/dbbkup/syslog.sql $BACKUPS/dbbkup/djigzo.sql

/usr/bin/7z a -t7z -ssc -m0=LZMA2:d64k:fb32 -ms=8m -mmt=8 -mx=1 $BACKUPS/$FILENAME_$TIMESTAMP.7z /etc/postfix/main.cf /etc/postfix/networks /etc/postfix/networks.db /etc/postfix/postscreen_access.cidr /etc/postfix/postscreen_access.cidr.db /etc/postfix/relay_domains /etc/postfix/relay_domains.db /etc/postfix/relay_passwd /etc/postfix/relay_passwd.db /etc/postfix/relay_recipients /etc/postfix/relay_recipients.db /etc/postfix/sender_access /etc/postfix/sender_access.db /etc/postfix/tls_policy /etc/postfix/tls_policy.db /etc/postfix/transport /etc/postfix/transport.db /etc/postfix/virtual /etc/postfix/virtual.db /etc/amavis/mynetworks /etc/amavis/black.list /etc/amavis/white.lst /etc/amavis/conf.d/15-av_scanners /etc/amavis/conf.d/50-user /etc/spamassassin/local.cf /opt/hermes /var/www/schedule /var/www/html/schedule /var/www/WEB-INF/railo/scheduler/scheduler.xml /var/www/html/WEB-INF/lucee/scheduler/scheduler.xml /mnt/data/amavis $BACKUPS/dbbkup/hermes.sql $BACKUPS/dbbkup/syslog.sql $BACKUPS/dbbkup/djigzo.sql  >> $BACKUPS/$FILENAME_$TIMESTAMP.log
