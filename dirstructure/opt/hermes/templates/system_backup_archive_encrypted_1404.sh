...
/usr/bin/mysqldump --user=root --password=***REDACTED-CRED*** --create-options --opt hermes>$BACKUPS/dbbkup/hermes.sql

/usr/bin/mysqldump --user=root --password=***REDACTED-CRED*** --create-options --opt Syslog>$BACKUPS/dbbkup/syslog.sql

/usr/bin/mysqldump --user=root --password=***REDACTED-CRED*** --create-options --opt djigzo>$BACKUPS/dbbkup/djigzo.sql
...
/usr/bin/openssl aes-256-cbc -pass pass:UIeocFdAeq3k/ewFUS/k2Ol+EEsRlNMHz21WWabsZXE= -a -salt -in $BACKUPS//hermes_sysarch_enc_$TIMESTAMP.rar -out $BACKUPS/hermes_sysarch_enc_$TIMESTAMP.backup
...
/bin/rm -rf $BACKUPS/hermes_sysarch_enc_$TIMESTAMP.rar
